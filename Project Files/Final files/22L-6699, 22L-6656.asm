...
printbricks:
			...
			brickprintloopouter:
				mov dl,[si]
				cmp dl,[bottomcol]
				je brickprintouterskip ;if current brickcol==bottomcol then dont print it`
				mov dx,[di]
				mov cx,10
						brickprintloopinner:
						;ax holds y, bx holds x, bx gets incremented 6 pixels every loop
						push dx
						push word[bricksize]
						push ax
						push dx
						...
						pop dx
						add dx,6
						loop brickprintloopinner
				brickprintouterskip:
				add di,2
				add si,1
				add ax,27
				sub bx,1
			jnz brickprintloopouter
			pop es
			pop si
			pop di
			pop dx
...
readimages:
			...
			lea dx,endfilename
			int 21h
			mov [endfilehandle],ax
			;read file and write bytes to 
			 mov ah,3fh
			 mov cx,64000
			 mov bx,[endfilehandle]
			 mov dx,[endScreen_buffer]
			 mov ds,dx
			 xor dx,dx
			 int 21h
			 ;close file
			...
			ret
;------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------					  			
startText:db 'Press E to Start Game'
startTextPrint:
			pusha
			mov ax,cs
			mov es,ax			
...
printpausescreen:
			pusha
			push es
			cmp word[cs:ispausedprinted],1
			je printpausescreenend
			mov word[cs:ispausedprinted],1
			mov ax,0xA000
			mov es,ax
			mov di,320*75+80
			mov dx,60
			...
			mov bp,pausemsg1;mov bp the offset of the string
			int 10h
			mov cx,20;length of string
			mov dh,14;y coordinate
...
;--------------------------------------------------------------------
pausescreen:
			cmp word[cs:ispaused],1
			je pausescreenskip
				mov word[cs:ispaused],1
				mov word[cs:ispausedprinted],0
					jmp pausescreenend
			pausescreenskip:
				mov word[cs:ispaused],0
			pausescreenend:
			ret
;--------------------------------------------------------------------
endmsg: db 'You died!'
...
printendmsg:
			...
			mov dl,15;x coordinate
			mov bp,endmsg;mov bp the offset of the string
			int 10h
			mov cx,6;length of string
			mov dh,20;y coordinate
			mov dl,15;x coordinate
			mov bp,endmsg2;mov bp the offset of the string
			int 10h
			mov cx,2;length of string
			mov dh,20;y coordinate
			mov dl,21;x coordinate
			mov bp,digits;mov bp the offset of the string
			int 10h
			popa
		    ret
...
sound_size:dw 11000
filehandle: dw 0
startfilename:db 'Start.txt',0
startfilehandle:dw 0
endfilename:db 'End.txt',0
endfilehandle:dw 0

stack_buffer: dw 0x7000
sound_buffer: dw 0x8000
startScreen_buffer: dw 0x6000
endScreen_buffer:dw 0x5000
...
carrotsize:dw 121
bricksize:dw 49
brickblue: db 44,44,44,44,44,44,254,53,44,44,44,44,53,254,53,53,44,44,53,53,254,53,53,53,53,53,53,254,53,53,53,53,53,53,254,54,54,54,54,54,54,254,55,55,55,55,55,55,254
brickskin: db 6,6,6,6,6,6,254,66,6,6,6,6,66,254,66,66,6,6,66,66,254,66,66,66,66,66,66,254,66,66,66,66,66,66,254,65,65,65,65,65,65,254,65,65,65,65,65,65,254
brickred:db 40,40,40,40,40,40,254,40,40,40,40,40,40,254,40,40,40,40,40,40,254,40,40,40,40,40,40,254,40,40,40,40,40,40,254,40,40,40,40,40,40,254,40,40,40,40,40,40,254
brickyellow: db 52,52,52,52,52,52,254,14,52,52,52,52,14,254,14,14,52,52,14,14,254,14,14,14,14,14,14,254,14,14,14,14,14,14,254,14,14,14,14,14,14,254,14,14,14,14,14,14,254
